...
const lrc_parser_1 = require("./lrc-parser");
 ...
 */
function isLyricExist(lyric, danmakuList, targetColor) {
    const lyricTimeMs = Math.round(lyric.time * 1000);
    const timeTolerance = 500; // 允许0.5秒的时间误差，以兼容服务器记录偏差
    return danmakuList.some(danmaku => danmaku.mode === 4 && // 仅比对底部弹幕 (mode: 4)
 ...
 * @returns 颜色的十进制表示。
 */
function parseColorToDecimal(hexColor) {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // 2. 解析LRC文件
        const allLyrics = (0, lrc_parser_1.parseLrc)(config.lrcPath);
        ...
        // 3. 登录并初始化API
        const biliApi = new bilibili_api_1.BilibiliApi();
        yield biliApi.ensureLogin();
        // 4. 获取视频信息 (CID)
        const { cid } = yield biliApi.getVideoInfo(config.bvid);
        // 5. 获取公共弹幕列表作为操作基准
        console.log('\n--- 步骤1: 获取公共弹幕列表作为基准 ---');
        console.log('正在以游客视角获取弹幕列表，以确保操作基于公开可见的内容...');
        const existingDanmaku = yield biliApi.getAnonymousDanmakuList(cid);
        console.log(`获取到 ${existingDanmaku.length} 条公共可见弹幕。`);
        const lyricsToSend = allLyrics.filter(lyric => !isLyricExist(lyric, existingDanmaku, danmakuColorDecimal));
        ...
        for (let i = 0; i < lyricsToSend.length; i++) {
            ...
            try {
                yield biliApi.sendDanmaku(lyric.text, lyric.time, config.bvid, cid, danmakuColorDecimal);
                console.log(`  -> 发送请求成功！`);
            }
            catch (error) {
            ...
            if (i < lyricsToSend.length - 1) {
                ...
                yield sleep(config.sendInterval);
            }
        }
        // 7. 进行最终双重校验
        console.log('\n--- 步骤3: 进行双重校验 ---');
        console.log('所有字幕发送任务已执行。等待50秒让服务器数据充分同步...');
        yield sleep(50000); // 等待服务器缓存刷新
        console.log('正在获取用户视角弹幕列表...');
        const finalUserDanmaku = yield biliApi.getDanmakuList(cid);
        console.log(`用户视角下有 ${finalUserDanmaku.length} 条弹幕。`);
        console.log('正在获取公共视角弹幕列表 (匿名)...');
        const finalPublicDanmaku = yield biliApi.getAnonymousDanmakuList(cid);
